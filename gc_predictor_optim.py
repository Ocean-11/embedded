...
def dataset_input_fn(mode, data_files):
    ...
    print('dataset_input_fn!!! - {}'.format(time.time()))
...
def load_dataset(mode, data_files):
    ...
    print('load_dataset!!! {}'.format(time.time()))
...
class image_predictor:
    def __init__(self, image_in, out_folder, image_width, model_dir, debug_image, show_images):
        ...
        tf.logging.set_verbosity(tf.logging.DEBUG)  # possible values - DEBUG / INFO / WARN / ERROR / FATAL - EMBEDDED
        ...
        print('---> Init image_predictor - init global variables = {}'.format(duration))
